...
const { pastebinKey } = require('../config.json');
...
async function getMessage(id, client, ch) {
  const res = client.channels.get(ch).fetchMessage(id)
    ...
    .catch((err) => { logger.error(err); throw new Error(`Message ${id} was not found`); });
  return res;
}
...
function getGuild(id, client) {
  ...
  if (!guild) throw new Error(`Guild ${id} was not found`);
  return guild;
}
...
/**
 * Handles discord object retrieval
 * @param {Array} args - array of given arguments
 * @param {Object} client - discord client
 * @param {string} ch - channel the message was received on
 * @returns {Object} discord object matching the arguments
 */
async function getSnowflake(args, client, ch) {
  let obj;
  switch (args[0]) {
    case 'message': obj = await getMessage(args[1], client, ch); break;
    ...
    default: throw new Error(`${args[0]} is not a valid type`);
  }
  return obj;
}
  ...
  execute(message, args, client) {
    ...
    getSnowflake(args, client, ch)
      ...
      .then((link) => {
        ...
        else throw new Error('Pastebin link was empty');
      })
      .catch((err) => {
        message.channel.send(`\`\`\`${err.message}\`\`\``); logger.warn('%o', err.message);
      });
  },
};
