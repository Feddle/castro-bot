...
function sortLeaderboard() {
  ...
  const newArr = arr.sort((a, b) => {
    const timeA = parseInt(Object.values(a)[0].replace(re, ''));
    const timeB = parseInt(Object.values(b)[0].replace(re, ''));
    return timeB - timeA;
  });
  return newArr;
}
...
function writeTime(id) {
  ...
  if (leaderboard[id] == undefined) leaderboard[id] = '00:00:00';
  const temp = JSON.parse(JSON.stringify(leaderboard));
  const timeBefore = leaderboard[id].split(':');
  let seconds = parseInt(timeBefore[2]) + waitingList[id].time.seconds;
  let minutes = parseInt(timeBefore[1]) + waitingList[id].time.minutes;
  let hours = parseInt(timeBefore[0]) + waitingList[id].time.hours;
  if (seconds >= 60) {
    minutes++;
    seconds -= 60;
  }
  ...
  const timeAfter = `${hours ? (hours > 9 ? hours : `0${hours}`) : '00'}:${
    minutes ? (minutes > 9 ? minutes : `0${minutes}`) : '00'}:${
    seconds > 9 ? seconds : `0${seconds}`}`;
  temp[id] = timeAfter;
  fs.writeFile(`${__dirname}/leaderboards/leaderboard_KO.json`, JSON.stringify(temp), (err) => {
...
function handle_KO(mentioned_users, message, client) {
  for (const [snowflake, user] of mentioned_users) {
    const id = snowflake;
...
module.exports = {
  ...
  execute(message, args, client) {
    fs.readFile(`${__dirname}/../leaderboards/leaderboard_KO.json`, (err, data) => {
      ...
      const mentioned_users = message.mentions.members;
